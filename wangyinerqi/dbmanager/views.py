...
class HostManage(View):
    ...
    def get(self,request):
        ...
        host_ip = []
        host_ip = SelectHost.objects.order_by("id")
        logger.info(host_ip)
        print host_ip
        ...
        if sid_d:
            ...
            render(request, 'hostmanage.html',locals())
        # delete_name = request.GET.get('dd_tablist', "")
        # if delete_name:
        # 分页
        # try:
        #     delete_page = request.GET.get('page', 1)
        # except PageNotAnInteger:
        #     delete_page = 1
        # delete_p = Paginator(host_ip, settings.PAGING, request=request)
        # all_host_ips = delete_p.page(delete_page)
        # if host_ip is not None:
        #     for all_host_ip in all_host_ips.object_list:
        #         logger.info(all_host_ip.id)
        #         print all_host_ip.id
        # print "-----------------删除主机--分页--end---------------------"
        ...
        if group_name:
            ...
            if host_list is not None:
                ...
                logger.info(host_list)
                print host_list
                logger.info("------------------选择执行任务主机分页----start--------------")
        else:
            host_list = all_hosts.filter(group=u'chredis')
        # 分页
        try:
            page_num = request.GET.get('page', 1)
        except (EmptyPage, PageNotAnInteger):
            page_num = 1
        paginator = Paginator(host_list, 2, request=request)
        host_add_lists = paginator.page(page_num)
        logger.info(host_add_lists)
        logger.info(host_add_lists.object_list)
        logger.info("-----------选择执行任务的主机---end------------------")
