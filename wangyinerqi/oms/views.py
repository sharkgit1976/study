...
class dayend(View):
    ...
    def post(self,request):
        ...
        if option == "id":
            command = "select id,usergroup,tabname,tabnamedesc,cmd,msg,dedate,stime from CH_DAYEND_LOG where id =\'%s\' order by to_number(id,'999999999') asc " % content
        elif option == "time":
            command = "select id,usergroup,tabname,tabnamedesc,cmd,msg,dedate,stime from CH_DAYEND_LOG where dedate =\'%s\' order by to_number(id,'999999999') asc " % content
	db = pg2_operate()
...
class reSelect(View):
    def post(self,request):
        ...
        logger.info(Id)
        logger.info(Dedate)
        command0 = "update CH_DAYEND_LOG set status = '1' where id = \'%s\' and  dedate = \'%s\'" % (Id , Dedate)
        logger.info("safsafsafasf")
        db = pg2_operate()
        db.update_table(settings.PG_DAYEND,command0)
        logger.info("qwerrqrwq")
        data={}
        data["recode"] ="success"
        # data["ret"] = ret
        logger.info(data)
...
class HostDesc(View):
    def get(self, request):
        ...
        return render(request, 'hostdesc.html', locals())
